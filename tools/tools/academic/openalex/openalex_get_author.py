"""
Tool for retrieving a specific author from OpenAlex by ID.

Fetches complete author information using OpenAlex identifiers.
"""
...
class OpenalexGetAuthorTool(Tool):
    """
    Tool for retrieving authors from OpenAlex.
    
    Description:
        Access OpenAlex authors API endpoint directly.
    ...
    def execute(self, context: ExecutionContext, params: Dict[str, Any]) -> Dict[str, Any]:
        """Retrieve OpenAlex author by ID."""
        import time
        ...
        # 构建URL - 使用author_id作为路径参数
        base_url = f"https://api.openalex.org/authors/{author_id}"
        
        # 准备其他查询参数
        other_params = {k: v for k, v in params.items() if k != 'author_id'}
        if other_params:
