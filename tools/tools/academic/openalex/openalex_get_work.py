"""
Tool for retrieving a specific work from OpenAlex by ID.

Fetches complete work information using OpenAlex identifiers.
"""
...
class OpenalexGetWorkTool(Tool):
    """
    Tool for retrieving works from OpenAlex.
    
    Description:
        Access OpenAlex works API endpoint directly.
    ...
    def execute(self, context: ExecutionContext, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 构建URL - 使用work_id作为路径参数
        base_url = f"https://api.openalex.org/works/{work_id}"
        
        # 准备其他查询参数
        other_params = {k: v for k, v in params.items() if k != 'work_id'}
        if other_params:
