"""
Tool for getting drug classes by Rxnorm drug name.

This tool retrieves drug classification information for a given drug name.
"""
...
class RxclassGetClassByRxnormDrugNameTool(Tool):
    """
    Tool for getting drug classes by Rxnorm drug name.
    
    Description:
        Retrieve drug classification information for a given drug name using the RxNAV REST API.
        Returns classes that the drug belongs to.
    
    Input Parameters:
        - drugName (str, required): Drug name to search for
    ...
    def execute(self, context: ExecutionContext, params: Dict[str, Any]) -> Dict[str, Any]:
        """Get drug classes by Rxnorm drug name."""
        try:
            # 构建URL和查询参数
            ...
            if hasattr(context, 'logger') and context.logger:
                context.logger.info(f"Making request to RxNAV API for classes by drug name")
