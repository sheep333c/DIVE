"""
Tool for getting Rxnorm properties for drug concepts.
...
class RxnormGetRxPropertyTool(Tool):
    """
    Tool for getting Rxnorm properties for drug concepts.
    ...
    def execute(self, context: ExecutionContext, params: Dict[str, Any]) -> Dict[str, Any]:
        """Get Rxnorm properties for a drug concept."""
        try:
            # 构建URL - rxcui在路径中，其他参数作为查询参数
            rxcui = params.get("rxcui", "")
            base_url = f"https://rxnav.nlm.nih.gov/REST/rxcui/{rxcui}/property.json"
            ...
            if hasattr(context, 'logger') and context.logger:
                context.logger.info(f"Making request to RxNAV API for Rxnorm properties")
