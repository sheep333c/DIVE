"""
Global pairwise sequence alignment tool using Bio.Align.PairwiseAligner.

This tool performs global pairwise sequence alignment with customizable scoring parameters.
"""
...
class Pairwise2GlobalAlignTool(Tool):
    """
    Tool for performing global pairwise sequence alignment.
    
    Uses Bio.Align.PairwiseAligner for global alignment with customizable
    match/mismatch scores and gap penalties.
    """
    ...
    def execute(self, context: ExecutionContext, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            ...
            params: Dictionary containing:
                ...
                - match_score (float, optional): Score for matching characters (default: 2.0)
                - mismatch_score (float, optional): Score for mismatching characters (default: -1.0)
                - open_gap_score (float, optional): Score for opening a gap (default: -0.5)
                - extend_gap_score (float, optional): Score for extending a gap (default: -0.1)
        ...
        try:
            ...
            match_score = params.get('match_score', 2.0)
            mismatch_score = params.get('mismatch_score', -1.0)
            open_gap_score = params.get('open_gap_score', -0.5)
            extend_gap_score = params.get('extend_gap_score', -0.1)
            ...
            # Perform global alignment with retry mechanism
            max_retries = 3
            retry_delay = 0.5  # seconds
            
            for attempt in range(max_retries):
                try:
                    # Create aligner and set parameters
